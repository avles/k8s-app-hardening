...
import yaml as pyyaml  # The PyYAML package
import git
            ...
            for file in files:
                if file.endswith(".yaml"):
                    ...
                    with open(filepath, 'r') as f:
                        try:
                            # Try to use ruamel first
                            content = yaml.load(f)
                        except ComposerError as e:
                            # If it's a multi-document file, use PyYAML
                            f.seek(0)
                            documents = list(pyyaml.load_all(f, Loader=pyyaml.FullLoader))

                            for content in documents:
                                modified |= apply_switches(content, switches, api_migration_pathway)  # using |= to accumulate True

                            with open(filepath, 'w') as f:
                                for content in documents:
                                    pyyaml.dump(content, f, Dumper=pyyaml.Dumper)
                            continue

                    # If it's not a multi-document file, proceed as usual
